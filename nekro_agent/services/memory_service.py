...
class MemoryService:
    ...
    async def save_memory(
        ...
        try:
            ...
            self.logger.exception(
                f"Error saving memory {project_id}/{memory_name}: {e}",
            )
            raise
    ...
    async def load_memory(
        ...
        try:
            ...
            raise KeyError(
                f"Memory '{memory_name}' not found in project '{project_id}'",
            )
    ...
    async def list_memories(
        ...
        try:
            ...
            self.logger.exception(f"Error listing memories for {project_id}: {e}")
            raise
    ...
    async def delete_memory(
        ...
        try:
            ...
            self.logger.warning(
                f"Memory not found for deletion: {project_id}/{memory_name}",
            )
            raise KeyError(
                f"Memory '{memory_name}' not found in project '{project_id}'",
            )
    ...
    async def search_memories(
        ...
        try:
            ...
            self.logger.exception(f"Error searching memories for {project_id}: {e}")
            raise
    ...
    async def get_memory_stats(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            most_accessed = sorted(
                memories, key=lambda m: m.access_count, reverse=True,
            )[:5]
            most_accessed_list = [
                {
                    "name": m.memory_name,
            ...
            recently_updated = sorted(
                memories, key=lambda m: m.updated_at, reverse=True,
            )[:5]
            ...
            self.logger.exception(f"Error getting memory stats for {project_id}: {e}")
            raise
    ...
    async def _update_search_index(self, memory: DBProjectMemory):
        ...
        try:
            ...
            self.logger.exception(
                f"Error updating search index for memory {memory.id}: {e}",
            )
    ...
    async def _update_tag_usage(self, project_id: str, tags: List[str]):
        ...
        try:
            ...
            self.logger.exception(f"Error updating tag usage for {project_id}: {e}")
    ...
    def _extract_snippets(
        self, content: str, search_terms: List[str], max_snippets: int = 3,
    ) -> List[str]:
