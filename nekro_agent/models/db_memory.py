...
class DBProjectMemory(Model):
    ...
    created_by = fields.CharField(
        max_length=255, null=True, description="Creator identifier",
    )
    ...
    priority = fields.IntField(
        default=0, description="Memory priority (higher = more important)",
    )
    ...
    # Content metadata
    content_length = fields.IntField(
        default=0, description="Content length in characters",
    )
    content_hash = fields.CharField(
        max_length=64, null=True, description="Content hash for change detection",
    )
...
class DBMemoryTag(Model):
    ...
    usage_count = fields.IntField(
        default=0, description="Number of memories using this tag",
    )
...
class DBMemoryLink(Model):
    ...
    id = fields.IntField(pk=True)
    source_memory = fields.ForeignKeyField(
        "models.DBProjectMemory", related_name="outgoing_links",
    )
    target_memory = fields.ForeignKeyField(
        "models.DBProjectMemory", related_name="incoming_links",
    )
...
class DBMemorySearch(Model):
    ...
    memory = fields.ForeignKeyField(
        "models.DBProjectMemory", related_name="search_entries",
    )
    ...
    language = fields.CharField(
        max_length=10, default="en", description="Content language",
    )
    last_indexed = fields.DatetimeField(
        auto_now=True, description="Last indexing timestamp",
    )
