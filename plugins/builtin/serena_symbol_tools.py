...
async def analyze_symbol_usage(
    ...
    try:
        ...
        find_adapter = serena_tool_registry.get_tool_adapter(
            "find_symbol", project_root,
        )
        ...
        for symbol in symbols[:5]:  # Limit to first 5 matches
            ...
            ref_adapter = serena_tool_registry.get_tool_adapter(
                "find_referencing_symbols", project_root,
            )
...
async def get_symbol_hierarchy(
    ...
    try:
        ...
        overview_adapter = serena_tool_registry.get_tool_adapter(
            "get_symbols_overview", project_root,
        )
...
async def refactor_symbol(
    ...
    try:
        ...
        results.append(
            f"Would rename symbol '{old_name_path}' to '{new_name}' in {relative_path}",
        )
...
def _analyze_symbol_dependencies(symbol_body: str, _file_path: str) -> Dict:
    """分析符号体中的依赖关系"""
    dependencies = {
        "imports": [],
        "function_calls": [],
