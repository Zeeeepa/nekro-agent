...
class CommandToolsConfig(ConfigBase):
    ...
    DANGEROUS_COMMANDS: List[str] = Field(
        default=[
            ...
            "dd if=", "mkfs", "fdisk", "parted", "kill -9",
            "killall", "pkill", "sudo rm", "chmod 777"
        ],
        title="危险命令列表",
    ...
    ALLOWED_COMMANDS: List[str] = Field(
        default=[
            ...
            "git", "npm", "pip", "python", "node", "java", "go",
            "make", "cmake", "cargo", "mvn", "gradle", "docker",
            "kubectl", "curl", "wget", "ping", "nslookup"
        ],
        title="允许的命令列表",
...
async def get_command_history(
    ...
    try:
        ...
        if filter_successful is not None:
            filtered_history = [
                entry for entry in _command_history
...
async def get_system_info(_ctx: AgentCtx = None) -> str:
    """获取系统信息
    ...
    try:
        ...
        system_info = {
            ...
            "environment": {
                ...
                "path": os.environ.get("PATH", "").split(os.pathsep)[:10]  # Limit PATH entries
            }
        }
