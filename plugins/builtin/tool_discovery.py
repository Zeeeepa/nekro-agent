...
_tool_usage_stats: Dict[str, Dict[str, Any]] = defaultdict(
    lambda: {
        "usage_count": 0,
        "success_count": 0,
        "error_count": 0,
...
async def list_all_tools(
    ...
    try:
        ...
        if category_filter and category_filter in config.TOOL_CATEGORIES:
            ...
            all_tools = {
                name: info for name, info in all_tools.items() if name in category_tools
            }
            ...
            return json.dumps(
                {
                    ...
                    "suggestions": _get_similar_tool_names(
                        tool_name, list(all_tools.keys()),
                    ),
                },
            )
                ...
                recommendations.append(
                    {
                        ...
                        "usage_example": _get_tool_examples(tool_name)[0]
                        if _get_tool_examples(tool_name)
                        else None,
                        "why_recommended": _explain_recommendation(
                            tool_name, task_keywords,
                        ),
                    },
                )
...
def _get_tool_difficulty(tool_name: str) -> str:
    ...
    if any(pattern in tool_name for pattern in ["execute", "batch", "advanced"]):
        return "advanced"
    if any(pattern in tool_name for pattern in ["symbol", "refactor", "analyze"]):
        return "intermediate"
    if any(pattern in tool_name for pattern in ["get", "list", "read", "search"]):
        return "beginner"
    return "intermediate"
...
def _get_similar_tool_names(tool_name: str, all_tool_names: List[str]) -> List[str]:
    ...
    return difflib.get_close_matches(tool_name, all_tool_names, n=3, cutoff=0.6)
...
def _explain_recommendation(tool_name: str, keywords: List[str]) -> str:
    ...
    if any(kw in ["文件", "读取"] for kw in keywords) and ("read" in tool_name or "file" in tool_name):
