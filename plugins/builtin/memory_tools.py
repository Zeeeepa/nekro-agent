...
class MemoryToolsConfig(ConfigBase):
    ...
    DEFAULT_MEMORY_TYPES: List[str] = Field(
        default=[
            "general", "onboarding", "task", "code_analysis", 
            "architecture", "bug_fix", "feature", "documentation"
        ],
        title="默认记忆类型",
...
async def search_memories(
    ...
    try:
        ...
        if not project_id:
            project_root = get_sandbox_project_root()
            project_id = os.path.basename(project_root) or plugin.get_config().DEFAULT_PROJECT_ID
...
async def get_memory_content(
    ...
    try:
        ...
        content = await memory_service.load_memory(
            project_id=project_id,
            memory_name=memory_name,
            update_access=True
        )
...
def _extract_auto_tags(content: str) -> List[str]:
    ...
    technical_terms = [
        ...
        'docker', 'kubernetes', 'react', 'vue', 'angular', 'python', 'javascript',
        'typescript', 'java', 'go', 'rust', 'c++', 'sql', 'nosql', 'mongodb',
        'postgresql', 'mysql', 'redis', 'nginx', 'apache', 'aws', 'azure', 'gcp'
    ]
